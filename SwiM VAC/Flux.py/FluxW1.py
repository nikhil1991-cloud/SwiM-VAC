...
for id in range (0,np.shape(Line)[0]):
  ...
  apc_w1 = apc[1].data['ApertureCorrect(w2,m2,w1)'][ind_apc][0][2]
  
  hdu = fits.open(path_to_Swift_segmented_images+str(Line[id])+'_UVW1_seg.fits')
  ...
  hdu = fits.open(path_to_Swift_raw_data+'CNTS/m'+str(Line[id])+'-w1d-cts.fits')
  hdr = hdu[0].header
  counts = hdu[0].data
  ...
  Nflux_NM = math.pow(10,9 - 0.4*NEWFLUX)*apc_w1
  Nfluxmag = 22.5 - 2.5*np.log10(Nflux_NM)


  hdu = fits.open(path_to_Swift_raw_data+'ERR/m'+Line[id]+'-w1d-cts-err.fits')
  err = hdu[0].data

  hdu = fits.open(path_to_Swift_raw_data+'EXP/m'+Line[id]+'-w1d-ex-dc.fits')
  exp = hdu[0].data

  Rate = (counts-sky_counts)/exp
  Fluxnmg1 = Rate*math.pow(10,9-zp1*0.4)
  MASK_W1 = np.zeros(np.shape(Fluxnmg1))
  MASK_W1[Fluxnmg1 == -inf]=1
  MASK_W1[Fluxnmg1== inf]=1
  MASK_W1[np.isnan(Fluxnmg1)]=1
  erate = err/exp
  ...
  hdu5 = fits.ImageHDU(MASK_W1)

  new_hdul = fits.HDUList([hdu0,hdu1,hdu2,hdu3,hdu4,hdu5])
  new_hdul.writeto(path_to_store_Swift_Flux_maps+str(Line[id])+'_UVW1_flx.fits')
    
  hdu = fits.open(path_to_store_Swift_Flux_maps+str(Line[id])+'_UVW1_flx.fits')
  hdr = hdu[0].header
  hdr['SIMPLE'] = simple
  hdr['BITPIX'] = bitpix
  hdr['NAXIS'] = naxis
  ...
  hdu.writeto(path_to_store_Swift_Flux_maps+str(Line[id])+'_UVW1_flx.fits',overwrite=True)
