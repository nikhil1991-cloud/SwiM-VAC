...
for id in range (0,np.shape(Line)[0]):
    ...
    hdu_counts = fits.open(get_pkg_data_filename(path_to_Swift_raw_data+'CNTS/m'+str(Line[id])+'-m2d-cts.fits'))[0]
    hdu_exp = fits.open(get_pkg_data_filename(path_to_Swift_raw_data+'EXP/m'+str(Line[id])+'-m2d-ex-dc.fits'))[0]
    hdu_err = fits.open(get_pkg_data_filename(path_to_Swift_raw_data+'ERR/m'+str(Line[id])+'-m2d-cts-err.fits'))[0]
    ...
    new_hdul.writeto(path_to_store_output_files+'UVM2_cutout_'+str(Line[id])+'.fits')
